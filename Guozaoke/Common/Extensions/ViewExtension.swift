...
public func topSafeAreaInset() -> UIEdgeInsets {
    ...
    if let insets = GuozaokeApp.window?.safeAreaInsets {
        result = insets
    } else {
        let safeAreaInsets = DeviceUtils.getSafeAreaInsets()
        result = UIEdgeInsets(top: safeAreaInsets.top, left: 0, bottom: safeAreaInsets.bottom, right: 0)
    }
    return result;
}
...
extension View {
    public func debug(_ force: Bool = false, _ color: Color = .green) -> some View {
        //        logger(Mirror(reflecting: self).subjectType)
